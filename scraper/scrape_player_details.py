...
async def main():
    ...
    all_players.update(load_index_movers())
    try:
    ...
    if blacklist:
        ...
        all_players = {pid: slug for pid, slug in all_players.items() if pid not in blacklist}
        print(f"Skipping {before - len(all_players)} known no-market players (blacklist)")
