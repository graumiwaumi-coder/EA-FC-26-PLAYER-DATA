...
md("## Step 5: Train + validate with MULTIPLE walk-forward folds\n\n"
   ...
   "time on only the data before that point and testing only on data after it (with a "
   "21-day embargo gap so no training label's forward-return window overlaps the test "
   "period). If the win rate is similar across folds, the edge is real and stable, not a "
...
for i in range(len(fold_edges) - 1):
    ...
    print(f"Fold {i+1}: train up to {train_end.date()}, test {test_start.date()}->{test_end.date()} "
